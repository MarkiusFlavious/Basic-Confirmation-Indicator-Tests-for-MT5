...
## More on iCustom and Trade Signal Function

### iCustom

You may come across some indicators that use input groups. When that happens, you will need to pass values into iCustom for them. If you look at the source code for Half Trend New Alert for example you will see the following input parameters:  
...
We can see that it has 2 input groups. For this specific indicator, the only input we actually care about is its amplitude, so we can hard code values for the rest of the inputs. In our EA's OnInit function we can assign our handle with the iCustom function like this:

```mql5
HalfTrendHandle = iCustom(_Symbol,Timeframe,"Half Trend New Alert.ex5",inpHTAmplitude,NULL,233,234,10,NULL,"alert.wav",3,3,false,false,false,false);
```

Remember that the Timeframe and inpHTAmplitude are defined as input variables.  

### Trade Signal Function

Some indicators might have colour changes that can be used for signals. A good example here would be the Supertrend indicator. You could either get your confirmation by checking if price moved above/below the Supertrend line or you could simply check if the line changed from red to green or vica versa.  

It is also not always necessary to use some indicators in the "normal" way. For example the RSI indicator is usually used to check if an asset is "Overbought" or "Oversold." Reversal trading is not recommended. A better way would be to use it as a 0 cross indicator and ignore the overbought/oversold zones entirely.
