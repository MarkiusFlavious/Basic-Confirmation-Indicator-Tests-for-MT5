...
int OnInit()
  {
   ...
   DiNapoliHandle = iCustom(_Symbol,Timeframe,"dinapolistochastic.ex5",FastK,SlowK,SlowD,Shift);
...
enTradeSignal CheckForSignal()
  {
   // All we actually care about here is the signal line crossing 50. At buffer pos 1.
   double SignalLineValues[];
   CopyBuffer(DiNapoliHandle,1,1,2,SignalLineValues);
   ArrayReverse(SignalLineValues);
   
   if ((SignalLineValues[0] > 50) && (SignalLineValues[1] < 50))
   {
    return buy_signal;
   }
   if ((SignalLineValues[0] < 50) && (SignalLineValues[1] > 50))
