...
int OnInit()
  {
   ...
   HeikenAshiSmoothedHandle = iCustom(_Symbol,Timeframe,"heiken_ashi_smoothed.ex5",MA_SMethod,SmLength,SmPhase);
...
enTradeSignal CheckForSignal()
  {
   // Simple colour change which is buffer 4. 0 is blue and 1 is red.
   double HeikenColourValue[];
   CopyBuffer(HeikenAshiSmoothedHandle,4,1,2,HeikenColourValue);
   ArrayReverse(HeikenColourValue);
   
   if ((HeikenColourValue[0] == 0) && (HeikenColourValue[1] == 1))
   {
    return buy_signal;
   }
   if ((HeikenColourValue[0] == 1) && (HeikenColourValue[1] == 0))
   {
    return sell_signal;
   }
