...
input int  inpCorrAnglePeriod = 35; // Period
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   CorrAngleHandle = iCustom(_Symbol,Timeframe,"Correlation Angle.ex5",inpCorrAnglePeriod);
...
enTradeSignal CheckForSignal()
  {
   // Simple 2 lines cross. Green Line Buffer 2, Red Line Buffer 3.
   double GreenLineValues[];
   CopyBuffer(CorrAngleHandle,2,1,2,GreenLineValues);
   ArrayReverse(GreenLineValues);
   
   double RedLineValues[];
   CopyBuffer(CorrAngleHandle,3,1,2,RedLineValues);
   ArrayReverse(RedLineValues);
   
   if ((GreenLineValues[0] > RedLineValues[0]) && (GreenLineValues[1] < RedLineValues[1]))
   {
    return buy_signal;
   }
   if ((GreenLineValues[0] < RedLineValues[0]) && (GreenLineValues[1] > RedLineValues[1]))
