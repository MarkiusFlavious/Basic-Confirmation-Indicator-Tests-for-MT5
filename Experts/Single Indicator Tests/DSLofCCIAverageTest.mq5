...
input group "DSL of CCI Average Inputs"
...
int OnInit()
  {
   ...
   DSLofCCIAverageHandle = iCustom(_Symbol,Timeframe,"Dsl CCI of average.ex5",inpCciPeriod,inpMaPeriod,inpMaMethod,inpPrice,inpSignalPeriod,inpZones);
...
enTradeSignal CheckForSignal()
  {
   // Same story with the other DSL of indicators.
   double ColourLineValues[];
   CopyBuffer(DSLofCCIAverageHandle,5,1,2,ColourLineValues);
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   if (inTrade == true)
   {
    if (PositionSelectByTicket(posTicket))
    {
     ...
     CopyBuffer(DSLofCCIAverageHandle,5,1,1,ColourLineValue);
