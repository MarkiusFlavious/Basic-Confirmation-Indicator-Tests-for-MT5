...
input int inpSuperHullPeriod = 12; // Hull period
input enSuperHullPrices inpSuperHullPrice = pr_median; // Price
input int inpSuperHullATRPeriod = 12; // ATR period
input double inpSuperHullATRMultiplier = 0.66; // ATR multiplier
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
int ATRChannelHandle;
int SupertrendHullHandle;
//+------------------------------------------------------------------+
//| Global Variables                                                 |
//+------------------------------------------------------------------+
int barTotal;
...
int OnInit()
  {
   ...
   SupertrendHullHandle = iCustom(_Symbol,Timeframe,"Downloads\\super_trend_hull.ex5",inpSuperHullPeriod,inpSuperHullPrice,inpSuperHullATRPeriod,inpSuperHullATRMultiplier);
...
enTradeSignal CheckForSignal()
  {
   double SupertrendHullColourValues[];
   CopyBuffer(SupertrendHullHandle,1,1,2,SupertrendHullColourValues);
   ArrayReverse(SupertrendHullColourValues);
   
   if ((SupertrendHullColourValues[0] == 0) && (SupertrendHullColourValues[1] == 1))
   {
    return buy_signal;
   }
   if ((SupertrendHullColourValues[0] == 1) && (SupertrendHullColourValues[1] == 0))
   {
    return sell_signal;
   }
