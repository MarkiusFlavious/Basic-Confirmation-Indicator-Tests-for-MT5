...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Half_Trend_Handle = iCustom(Pair,Timeframe,"Half Trend New Alert.ex5",Half_Trend_Amp,"Arrow",233,234,10,"Alerts","alert.wav",3,3,false,false,false,false);
...
//| - Colour Change Buffer = 1:                                      |
//|   - Red = 0, Blue = 1                                            |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_colour[];
   CopyBuffer(Half_Trend_Handle,1,1,2,line_colour);
   ArrayReverse(line_colour);
   
   if (line_colour[0] == 1){
      if (line_colour[1] == 0) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (line_colour[0] == 0){
