...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Supertrend_Hull_Handle = iCustom(Pair,Timeframe,"Downloads\\super_trend_hull.ex5",Hull_Period,Hull_Price,ST_Atr_Period,ST_Atr_Multiplier);
...
//| - Line colour buffer = 1 :                                       |
//|   - Green = 0, red = 1                                           |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_colour[];
   CopyBuffer(Supertrend_Hull_Handle,1,1,2,line_colour);
   ArrayReverse(line_colour);
   
   if (line_colour[0] == 0){
      if (line_colour[1] == 1) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (line_colour[0] == 1){
      if (line_colour[1] == 0) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
...
void CSingleIndicatorTester::TrailStop(void){
   if (TradePosition.Trail_Stop && TradePosition.Modified){
      ...
      CopyBuffer(Supertrend_Hull_Handle,0,1,1,supertrend_value);
      int digits = (int)SymbolInfoInteger(Pair,SYMBOL_DIGITS);
      double new_stop;
      ...
      if (TradePosition.Trade_Method == CLOSE_PARTIAL){
         if (PositionSelectByTicket(TradePosition.TicketA)){
            if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY){
               if (Trail_With_ST) new_stop = NormalizeDouble(supertrend_value[0],digits);
