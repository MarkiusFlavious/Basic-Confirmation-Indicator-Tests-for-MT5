...
enum COR_WIL_EMA_CHG_COLOR {
   ON_SLOPE,  // change color on slope change
   ON_LEVELS,  // Change color on outer levels cross
   ON_MID, // Change color on middle level cross
   ON_AVG    // Change color on average value cross
};
//+------------------------------------------------------------------+
...
// Input Parameters:
   ...
   // Corrected Wilder EMA Inputs:
   int Avg_Period;
   ENUM_APPLIED_PRICE Avg_Price;
   int Correction_Period;
   COR_WIL_EMA_CHG_COLOR Color_On;
   int Floating_Period;
   double Floating_Upper;
   double Floating_Lower;

// Indicator Handles:
   int ATR_Channel_Handle;
   int Cor_Wil_EMA_Handle;
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
                                               ...
                                               int floating_period,
                                               double floating_upper,
                                               double floating_lower){ // Add Indicator Inputs
   // Initialize Inputs
   Pair = pair;
   Timeframe = timeframe;
   ...
   Color_On = color_on;
   Floating_Period = floating_period;
   Floating_Upper = floating_upper;
   Floating_Lower = floating_lower;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Cor_Wil_EMA_Handle = iCustom(Pair,Timeframe,"Corr Wilder EMA (fl).ex5",Avg_Period,Avg_Price,Correction_Period,Color_On,Floating_Period,Floating_Upper,Floating_Lower);
