...
// Input Parameters:
   ...
   // Trend Rider Inputs:
   bool Trail_With_Trend_Rider;
   int Trend_Rider_ATR_Length;
   double Trend_Rider_ATR_Multiplier;
   ENUM_APPLIED_PRICE Trend_Rider_ATR_App_Price;
   int Trend_Rider_RSI_Period;
   ENUM_APPLIED_PRICE Trend_Rider_RSI_App_Price;
   int Trend_Rider_MACD_Fast_EMA;
   int Trend_Rider_MACD_Slow_EMA;
   int Trend_Rider_MACD_Signal_EMA;
   ENUM_APPLIED_PRICE Trend_Rider_MACD_App_Price;

// Indicator Handles:
   int ATR_Channel_Handle;
   int Trend_Rider_Handle;
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   Trail_With_Trend_Rider = trail_with_trend_rider;
   
   Trend_Rider_ATR_Length = trend_rider_atr_length;
   Trend_Rider_ATR_Multiplier = trend_rider_atr_multiplier;
   Trend_Rider_ATR_App_Price = trend_rider_atr_app_price;
   Trend_Rider_RSI_Period = trend_rider_rsi_period;
   Trend_Rider_RSI_App_Price = trend_rider_rsi_app_price;
   Trend_Rider_MACD_Fast_EMA = trend_rider_macd_fast_ema;
   Trend_Rider_MACD_Slow_EMA = trend_rider_macd_slow_ema;
   Trend_Rider_MACD_Signal_EMA = trend_rider_macd_signal_ema;
   Trend_Rider_MACD_App_Price = trend_rider_macd_app_price;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Trend_Rider_Handle = iCustom(Pair,Timeframe,"Trend Rider.ex5",Trend_Rider_ATR_Length,Trend_Rider_ATR_Multiplier,Trend_Rider_ATR_App_Price,
                                Trend_Rider_RSI_Period,Trend_Rider_RSI_App_Price,Trend_Rider_MACD_Fast_EMA,Trend_Rider_MACD_Slow_EMA,
                                Trend_Rider_MACD_Signal_EMA,Trend_Rider_MACD_App_Price,false,false);
...
//| - Trend Rider has 10 buffers :                                   |
//|   - Candle Color Buffer = 4 :                                    |
//|     - Gray = 0, Green = 1, Red = 2                               |
//|   - Supertrend Line Value Buffer = 5                             |
//|   - Supertrend Color Buffer = 6 :                                |
//|     - Blue = 0, Red = 1                                          |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double bar_color[];
   CopyBuffer(Trend_Rider_Handle,4,1,2,bar_color);
   ArrayReverse(bar_color);
   
   if (bar_color[0] == 1){
      if (bar_color[1] == 0 || bar_color[1] == 2) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (bar_color[0] == 2){
      if (bar_color[1] == 0 || bar_color[1] == 1) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
...
void CSingleIndicatorTester::TrailStop(void){
   if (TradePosition.Trail_Stop && TradePosition.Modified){
      double atr_upper[],atr_lower[],supertrend_value[];
      CopyBuffer(ATR_Channel_Handle,1,1,1,atr_upper);
      CopyBuffer(ATR_Channel_Handle,2,1,1,atr_lower);
      CopyBuffer(Trend_Rider_Handle,5,1,1,supertrend_value);
      int digits = (int)SymbolInfoInteger(Pair,SYMBOL_DIGITS);
      double new_stop;
      ...
      if (TradePosition.Trade_Method == CLOSE_PARTIAL){
         if (PositionSelectByTicket(TradePosition.TicketA)){
            if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY){
               if (Trail_With_Trend_Rider) new_stop = NormalizeDouble(supertrend_value[0],digits);
               else new_stop = NormalizeDouble(atr_lower[0],digits);
               if (new_stop > PositionGetDouble(POSITION_SL)) trade.PositionModify(TradePosition.TicketA,new_stop,0);
            }
            else if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL){
               if (Trail_With_Trend_Rider) new_stop = NormalizeDouble(supertrend_value[0],digits);
               else new_stop = NormalizeDouble(atr_upper[0],digits);
               if (new_stop < PositionGetDouble(POSITION_SL)) trade.PositionModify(TradePosition.TicketA,new_stop,0);
            }
         }
      }
