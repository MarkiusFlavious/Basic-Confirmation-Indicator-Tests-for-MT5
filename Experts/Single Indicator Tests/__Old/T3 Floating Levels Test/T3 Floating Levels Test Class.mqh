...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   T3_Calc_Mode = t3_calc_mode;
   T3_App_Price = t3_app_price;
   Colour_Chg = colour_chg;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   T3_Floating_Levels_Handle = iCustom(Pair,Timeframe,"T3 floating levels (2).ex5",T3_Period,T3_Volume_Factor,T3_Calc_Mode,T3_App_Price,
                                       Colour_Chg,Floating_Period,Floating_Upper,Floating_Lower);
...
//| - Line colour change buffer = 4                                  |
//|   - Gray = 0, green = 1, orange = 2                              |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_colour[];
   CopyBuffer(T3_Floating_Levels_Handle,4,1,2,line_colour);
