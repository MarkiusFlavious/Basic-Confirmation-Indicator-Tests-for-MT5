...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   TM_CCI_Period = tm_cci_period;
   TM_ATR_Period = tm_atr_period;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Trendmagic_Handle = iCustom(Pair,Timeframe,"Downloads\\trendmagic.ex5",TM_CCI_Period,TM_ATR_Period);
...
//| - Color change buffer = 1 :                                      |
//|   - Blue = 0, red = 1                                            |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_color[];
   CopyBuffer(Trendmagic_Handle,1,1,2,line_color);
   ArrayReverse(line_color);
   
   if (line_color[0] == 0){
      if (line_color[1] == 1) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (line_color[0] == 1){
      if (line_color[1] == 0) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
