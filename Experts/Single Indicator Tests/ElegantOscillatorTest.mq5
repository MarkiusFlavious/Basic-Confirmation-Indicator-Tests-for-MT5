...
int OnInit()
  {
   ...
   ElegantOscillatorHandle = iCustom(_Symbol,Timeframe,"Elegant oscillator.ex5",inpBandEdge,inpOscillatorPeriod,inpPrice);
...
enTradeSignal CheckForSignal()
  {
   // 0 cross oscillator
   double ElegantValues[];
   CopyBuffer(ElegantOscillatorHandle,0,1,2,ElegantValues);
   ArrayReverse(ElegantValues);
   
   if ((ElegantValues[0] > 0) && (ElegantValues[1] < 0))
   {
    return buy_signal;
   }
   if ((ElegantValues[0] < 0) && (ElegantValues[1] > 0))
