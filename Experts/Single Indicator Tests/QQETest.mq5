...
input int inpQQERsiPeriod = 8; // QQE RSI period
input int inpQQeRsiSmoothingFactor = 1; // QQE RSI smoothing factor
input double inpQQEWPFast = 3; // QQE Fast period
input double inpQQEWPSlow = 4.236; // QQE Slow period
input ENUM_APPLIED_PRICE inpQQEPrice = PRICE_CLOSE; // QQE Applied Price 
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   QQEHandle = iCustom(_Symbol,Timeframe,"QQE.ex5",inpQQERsiPeriod,inpQQeRsiSmoothingFactor,inpQQEWPFast,inpQQEWPSlow,inpQQEPrice);
...
enTradeSignal CheckForSignal()
  {
   double QQEFastValues[];
   CopyBuffer(QQEHandle,0,1,2,QQEFastValues);
   ArrayReverse(QQEFastValues);
   
   double QQESlowValues[];
   CopyBuffer(QQEHandle,1,1,2,QQESlowValues);
   ArrayReverse(QQESlowValues);
   
   if ((QQEFastValues[0] > QQESlowValues[0]) && (QQEFastValues[1] < QQESlowValues[1]))
   {
    return buy_signal;
   }
   if ((QQEFastValues[0] < QQESlowValues[0]) && (QQEFastValues[1] > QQESlowValues[1]))
