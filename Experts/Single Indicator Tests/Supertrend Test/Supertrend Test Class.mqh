...
#include <Custom/Confirmation Indicators/Supertrend.mqh>
//+------------------------------------------------------------------+
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   C1_Supertrend.Pair = pair;
   C1_Supertrend.Timeframe = timeframe;
   C1_Supertrend.Trail_With_ST = trail_with_st;
   C1_Supertrend.Super_Period = super_period;
   C1_Supertrend.Super_Multiplier = super_multiplier;
...
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   return C1_Supertrend.CheckSignal(1);
}
//+------------------------------------------------------------------+
...
void CSingleIndicatorTester::TrailStop(void){
   if (TradePosition.Trail_Stop && TradePosition.Modified){
      ...
      int digits = (int)SymbolInfoInteger(Pair,SYMBOL_DIGITS);
      double st_value[];
      C1_Supertrend.GetBufferValues(2,1,1,st_value);
      
      if (TradePosition.Trade_Method == CLOSE_PARTIAL){
         if (PositionSelectByTicket(TradePosition.TicketA)){
            if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY){
               double new_stop;
               if (C1_Supertrend.Trail_With_ST) new_stop = NormalizeDouble(st_value[0],digits);
