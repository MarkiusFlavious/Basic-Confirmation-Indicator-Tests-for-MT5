...
int OnInit()
  {
   ...
   DynamicTrendHandle = iCustom(_Symbol,Timeframe,"Downloads\\Dynamic_Trend.ex5",InpPeriod,InpPercent);
...
enTradeSignal CheckForSignal()
  {
   //Colour Buffer at 1. 0 is bullish 1 is bearish.
   double DynamicTrendColourValues[];
   CopyBuffer(DynamicTrendHandle,1,1,2,DynamicTrendColourValues);
   ArrayReverse(DynamicTrendColourValues);
   
   if ((DynamicTrendColourValues[0] == 0) && (DynamicTrendColourValues[1] == 1))
   {
    return buy_signal;
   }
   if ((DynamicTrendColourValues[0] == 1) && (DynamicTrendColourValues[1] == 0))
   {
    return sell_signal;
   }
