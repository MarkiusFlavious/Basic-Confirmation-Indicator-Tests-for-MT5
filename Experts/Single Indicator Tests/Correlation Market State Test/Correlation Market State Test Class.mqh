...
// Input Parameters:
   ...
   // Indicator:
   CorrelationMarketState CorMarketState;
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   CorMarketState.Pair = pair;
   CorMarketState.Timeframe = timeframe;
   CorMarketState.Cor_State_Period = cor_state_period;
...
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   return CorMarketState.CheckSignal(1);
}
//+------------------------------------------------------------------+
