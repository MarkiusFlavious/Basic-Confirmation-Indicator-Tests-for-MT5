...
int OnInit()
  {
   ...
   DsluRSIofAverageHandle = iCustom(_Symbol,Timeframe,"Dslu RSI of average.ex5",inpRsiPeriod,inpMaPeriod,inpMaMethod,inpPrice,inpSignalPeriod,inpZones);
...
enTradeSignal CheckForSignal()
  {
   // RSI Line Colour Buffer is at 5. 0 is silver, 1 is green and 2 is orange
   // Exit at silver.
   double ColourLineValues[];
   CopyBuffer(DsluRSIofAverageHandle,5,1,2,ColourLineValues);
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   if (inTrade == true)
   {
    if (PositionSelectByTicket(posTicket))
    {
     ...
     CopyBuffer(DsluRSIofAverageHandle,5,1,1,ColourLineValue);
