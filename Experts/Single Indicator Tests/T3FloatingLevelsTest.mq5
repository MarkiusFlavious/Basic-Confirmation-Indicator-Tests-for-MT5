...
int OnInit()
  {
   ...
   T3FloatingLevelsHandle = iCustom(_Symbol,Timeframe,"T3 floating levels (2).ex5",T3Period,T3Hot,T3Original,T3Price,ColorOn,FlPeriod,FlUp,FlDown);
...
enTradeSignal CheckForSignal()
  {
   //3 Colour filter scheme. Buffer value 4: 0 is Gray, 1 is Green, 2 is Orange
   double ColourLineValues[];
   CopyBuffer(T3FloatingLevelsHandle,4,1,2,ColourLineValues);
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   if (inTrade == true)
   {
    if (PositionSelectByTicket(posTicket))
    {
     ...
     CopyBuffer(T3FloatingLevelsHandle,4,1,1,ColourLineValue);
