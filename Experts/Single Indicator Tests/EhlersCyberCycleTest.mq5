...
input group "Ehlers Cyber Cycle Input"
input double alpha = 0.07; //Alpha
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
int ATRChannelHandle;
int EhlersCyberCycleHandle;
//+------------------------------------------------------------------+
//| Global Variables                                                 |
//+------------------------------------------------------------------+
int barTotal;
...
int OnInit()
  {
   ...
   EhlersCyberCycleHandle = iCustom(_Symbol,Timeframe,"EhlersCyberCycle.ex5",alpha);
...
enTradeSignal CheckForSignal()
  {
   // Buffer 1. 1 is Green and 2 is red.
   double ColourLineValues[];
   CopyBuffer(EhlersCyberCycleHandle,1,1,2,ColourLineValues);
   ArrayReverse(ColourLineValues);
   
   if ((ColourLineValues[0] == 1) && (ColourLineValues[1] == 2))
   {
    return buy_signal;
   }
   if ((ColourLineValues[0] == 2) && (ColourLineValues[1] == 1))
   {
    return sell_signal;
   }
