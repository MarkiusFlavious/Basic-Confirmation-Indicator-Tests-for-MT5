...
input group "Reflex Input"
input int inpReflexPeriod = 50; // Reflex period
input enStratType StratType = colour_change; // Strategy type
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   RefexHandle = iCustom(_Symbol,Timeframe,"Reflex.ex5",inpReflexPeriod);
...
void OnTick()
  {
   ...
   if (barTotal != barTotalUpdated)
   {
    ...
    enTradeSignal tradeSignal = CheckForSignal(StratType);
...
//+------------------------------------------------------------------+
enTradeSignal CheckForSignal(enStratType Strategy)
  {
   if (Strategy == colour_change)
   {
    double ColourValues[];
    CopyBuffer(RefexHandle,1,1,2,ColourValues);
    ArrayReverse(ColourValues);
    
    if ((ColourValues[0] == 0) && (ColourValues[1] == 1))
    {
     return buy_signal;
    }
    if ((ColourValues[0] == 1) && (ColourValues[1] == 0))
    {
     return sell_signal;
    }
   }
   ...
   {
    double ReflexValues[];
    CopyBuffer(RefexHandle,0,1,2,ReflexValues);
    ArrayReverse(ReflexValues);
    
    if ((ReflexValues[0] > 0) && (ReflexValues[1] < 0))
    {
     return buy_signal;
    }
    if ((ReflexValues[0] < 0) && (ReflexValues[1] > 0))
