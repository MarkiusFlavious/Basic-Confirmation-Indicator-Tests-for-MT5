...
int OnInit()
  {
   ...
   CorrWilderEMAHandle = iCustom(_Symbol,Timeframe,"Corr Wilder EMA (fl).ex5",inpPeriod,inpPrice,inpCorrectionPeriod,inpColorOn,inpFlPeriod,inpFlUp,inpFlDown);
...
enTradeSignal CheckForSignal()
  {
   // Buffer 5 and 6 for the corrected average. 6 is colour. 0 is Gray, 1 is Green and 2 is Red.
   double CorrEMAColourValues[];
   CopyBuffer(CorrWilderEMAHandle,6,1,2,CorrEMAColourValues);
   ArrayReverse(CorrEMAColourValues);
   
   if ((CorrEMAColourValues[0] == 1) && ((CorrEMAColourValues[1] == 2) || (CorrEMAColourValues[1] == 0)))
   {
    return buy_signal;
   }
   if ((CorrEMAColourValues[0] == 2) && ((CorrEMAColourValues[1] == 1) || (CorrEMAColourValues[1] == 0)))
   {
    return sell_signal;
   }
