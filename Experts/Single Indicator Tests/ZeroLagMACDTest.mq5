...
input group "Zero Lag MACD Inputs"
...
int OnInit()
  {
   ...
   ZeroLagMACDHandle = iCustom(_Symbol,Timeframe,"zerolag_macd.ex5",inpFastPeriod,inpSlowPeriod,inpSignalPeriod,inpAppliedPrice);
...
enTradeSignal CheckForSignal()
  {
   double ZeroLagMACDSignalValues[];
   CopyBuffer(ZeroLagMACDHandle,1,1,2,ZeroLagMACDSignalValues);
   ArrayReverse(ZeroLagMACDSignalValues);
   
   if ((ZeroLagMACDSignalValues[0] > 0) && (ZeroLagMACDSignalValues[1] < 0))
   {
    return buy_signal;
   }
   if ((ZeroLagMACDSignalValues[0] < 0) && (ZeroLagMACDSignalValues[1] > 0))
