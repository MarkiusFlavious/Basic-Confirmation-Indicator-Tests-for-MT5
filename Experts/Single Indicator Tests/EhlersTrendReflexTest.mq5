...
enum enIndiStrat
  {
   twoLine_Cross, // 2 Line Cross
   Blue_Zero, // Blue Line Zero Cross
   Red_Zero // Red Line Zero Cross
  };
//+------------------------------------------------------------------+
//| Inputs                                                           |
//+------------------------------------------------------------------+
...
input group "Ehlers Trend Reflex"
input int inpLength = 20;
input enIndiStrat inpStrat = Blue_Zero; // Version
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
int ATRChannelHandle;
int EhlersTrendReflexHandle;
//+------------------------------------------------------------------+
//| Global Variables                                                 |
//+------------------------------------------------------------------+
int barTotal;
...
int OnInit()
  {
   ...
   EhlersTrendReflexHandle = iCustom(_Symbol,Timeframe,"EhlersTrendReflex.ex5",inpLength);
...
void OnTick()
  {
   ...
   if (barTotal != barTotalUpdated)
   {
    ...
    enTradeSignal tradeSignal = CheckForSignal(inpStrat);
...
//+------------------------------------------------------------------+
enTradeSignal CheckForSignal(enIndiStrat StratVersion)
  {
   // 3 ways to do this. 
   // 1. Blue line crosses red line, up for buy, down for sell
   // 2. Blue line crosses 0, up for buy, down for sell
   // 3. Red line crosses 0, up for buy, down for sell
   
   double BlueLineValues[];
   CopyBuffer(EhlersTrendReflexHandle,1,1,2,BlueLineValues);
   ArrayReverse(BlueLineValues);
   
   double RedLineValues[];
   CopyBuffer(EhlersTrendReflexHandle,0,1,2,RedLineValues);
   ArrayReverse(RedLineValues);
   
   // 2 Line Cross
   if (StratVersion == twoLine_Cross)
   {
    if ((BlueLineValues[0] > RedLineValues[0]) && (BlueLineValues[1] < RedLineValues[1]))
    {
     return buy_signal;
    }
    if ((BlueLineValues[0] < RedLineValues[0]) && (BlueLineValues[1] > RedLineValues[1]))
   ...
   // Blue Line Zero Cross
   if (StratVersion == Blue_Zero)
   {
    if ((BlueLineValues[0] > 0) && (BlueLineValues[1] < 0))
    {
     return buy_signal;
    }
    if ((BlueLineValues[0] < 0) && (BlueLineValues[1] > 0))
   ...
   // Red Line Zero Cross
   if (StratVersion == Red_Zero)
   {
    if ((RedLineValues[0] > 0) && (RedLineValues[1] < 0))
    {
     return buy_signal;
    }
    if ((RedLineValues[0] < 0) && (RedLineValues[1] > 0))
