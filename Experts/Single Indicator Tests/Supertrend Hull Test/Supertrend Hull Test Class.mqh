...
#include <Custom/Confirmation Indicators/Supertrend Hull.mqh>
//+------------------------------------------------------------------+
...
// Input Parameters:
   ...
   // Indicator:
   SupertrendHull C1_SupertrendHull;
...
// Public Function/Constructor/Destructor Declaration:
                        CSingleIndicatorTester(string pair,
                                               ...
                                               int st_atr_period,
                                               double st_atr_multiplier); // <<< Declare Indicator Inputs Here >>>
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   C1_SupertrendHull.Pair = pair;
   C1_SupertrendHull.Timeframe = timeframe;
   C1_SupertrendHull.Trail_With_ST = trail_with_st;
   C1_SupertrendHull.Hull_Period = hull_period;
   C1_SupertrendHull.Hull_Price = hull_price;
   C1_SupertrendHull.ST_Atr_Period = st_atr_period;
   C1_SupertrendHull.ST_Atr_Multiplier = st_atr_multiplier;
...
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   return C1_SupertrendHull.CheckSignal(1);
}
//+------------------------------------------------------------------+
...
void CSingleIndicatorTester::TrailStop(void){
   if (TradePosition.Trail_Stop && TradePosition.Modified){
      ...
      int digits = (int)SymbolInfoInteger(Pair,SYMBOL_DIGITS);
      double st_hull_value[];
      C1_SupertrendHull.GetBufferValues(0,1,1,st_hull_value);
      
      if (TradePosition.Trade_Method == CLOSE_PARTIAL){
         if (PositionSelectByTicket(TradePosition.TicketA)){
            if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY){
                double new_stop;
                if (C1_SupertrendHull.Trail_With_ST) new_stop = NormalizeDouble(st_hull_value[0],digits);
