...
input int inpTrendRiderAtrLength = 10; // ATR calculaion period
input double inpTrendRiderAtrMultiplier = 3.0; // ATR multiplier
input ENUM_APPLIED_PRICE inpTrendRiderAtrAppliedPrice = PRICE_CLOSE; // AtrAppliedPrice to use
input int inpTrendRiderRsiPeriod = 14; // Rsi Period
input ENUM_APPLIED_PRICE inpTrendRiderRsiAppliedPrice = PRICE_CLOSE; // Rsi Applied AtrAppliedPrice
input int inpTrendRiderMacdFastEMA = 12; // Macd fast period
input int inpTrendRiderMacdSlowEMA = 26; // Macd slow period
input int inpTrendRiderSignalEMA = 9; // Macd Signal period
input ENUM_APPLIED_PRICE inpTrendRiderMacdAppliedPrice = PRICE_CLOSE; // Applied AtrAppliedPrice
input bool inpTrendRiderAlertOn = false; // Alerts On
input bool inpTrendRiderSoundON = false; // Show Alert Message
input bool inpTrendRiderEmailON = false; // Send Email Alerts
input bool inpTrendRiderPushNotificationON = false; // Send Push Notifications
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
int ATRChannelHandle;
int TrendRiderHandle;
//+------------------------------------------------------------------+
//| Global Variables                                                 |
//+------------------------------------------------------------------+
int barTotal;
...
int OnInit()
  {
   ...
   // Trend Rider has 13 inputs and no input groups
   TrendRiderHandle = iCustom(_Symbol,Timeframe,"Trend Rider.ex5",inpTrendRiderAtrLength,inpTrendRiderAtrMultiplier,inpTrendRiderAtrAppliedPrice,inpTrendRiderRsiPeriod,inpTrendRiderRsiAppliedPrice,inpTrendRiderMacdFastEMA,
   inpTrendRiderMacdSlowEMA,inpTrendRiderSignalEMA,inpTrendRiderMacdAppliedPrice,inpTrendRiderAlertOn,inpTrendRiderSoundON,inpTrendRiderEmailON,inpTrendRiderPushNotificationON);
...
enTradeSignal CheckForSignal()
  {
   // Trend Rider has 10 buffers. Candle colour is set at buffer value 4. 0 is Gray, 2 is Red and 1 is Green
   // It also has a built in Supertrend with value buffer at 5 and colour at 6. 1 is Red and 0 is Blue.
   
   double TRBarColourValues[];
   CopyBuffer(TrendRiderHandle,4,1,2,TRBarColourValues);
   ArrayReverse(TRBarColourValues);
   
   if ( (TRBarColourValues[0] == 1) && ((TRBarColourValues[1] == 0) || (TRBarColourValues[1] == 2)) )
   {
    return buy_signal;
   }
   if ( (TRBarColourValues[0] == 2) && ((TRBarColourValues[1] == 0) || (TRBarColourValues[1] == 1)) )
   {
    return sell_signal;
   }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   {
    double TRBarColourValues[];
    CopyBuffer(TrendRiderHandle,4,1,1,TRBarColourValues);
   
    if (PositionSelectByTicket(posTicket))
    {
     if (TRBarColourValues[0] == 0)
     {
