...
input int inpTTFPeriod = 15; // Period
input int inpTTFT3Period = 5; // T3 period
input double inpTTFT3Hot = 0.7; // T3 hot 
input enT3Type inpTTFT3Type = t3_fulksmat; // T3 type
input double inpTTFLevels = 100; // Levels at +- (nnn)
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   TTFHandle = iCustom(_Symbol,Timeframe,"Downloads\\Trend trigger factor.ex5",inpTTFPeriod,inpTTFT3Period,inpTTFT3Hot,inpTTFT3Type,inpTTFLevels);
...
enTradeSignal CheckForSignal()
  {
   // The cleaner signals come from the levels line crossing 0 from 100 to -100 and vice versa.
   // Values at buffer 2
   double TTFLevelValues[];
   CopyBuffer(TTFHandle,2,1,2,TTFLevelValues);
   ArrayReverse(TTFLevelValues);
   
   if ((TTFLevelValues[0] > 0) && (TTFLevelValues[1] < 0))
   {
    return buy_signal;
   }
   if ((TTFLevelValues[0] < 0) && (TTFLevelValues[1] > 0))
