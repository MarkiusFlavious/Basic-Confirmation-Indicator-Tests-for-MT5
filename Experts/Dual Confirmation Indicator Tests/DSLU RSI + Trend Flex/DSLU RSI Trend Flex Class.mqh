...
#include <Custom/Confirmation Indicators/DSLU RSI Average.mqh>
#include <Custom/Confirmation Indicators/Trend Flex X2.mqh>
//+------------------------------------------------------------------+
...
// Indicators:
   DSLURSIAverage C1_DSLURSIAverage;
   TrendFlexX2 C2_TrendFlexX2;
...
// Public Function/Constructor/Destructor Declaration:
                        CDualIndicatorTest(string pair,
                                           ...
                                           int dslu_rsi_lookback_tolerance,
                                           bool dslu_rsi_is_exit,
                                           int trendflex_fast_period,
                                           int trendflex_slow_period,
                                           DUAL_MODE trendflex_method,
                                           int trendflex_lookback_tolerance,
                                           bool trendflex_is_exit);
...
CDualIndicatorTest::CDualIndicatorTest(string pair,
   ...
   C1_DSLURSIAverage.Pair = pair;
   C1_DSLURSIAverage.Timeframe =timeframe;
   C1_DSLURSIAverage.DSLU_RSI_Period = dslu_rsi_period;
   C1_DSLURSIAverage.DSLU_MA_Period = dslu_ma_period;
   C1_DSLURSIAverage.DSLU_MA_Method = dslu_ma_method;
   C1_DSLURSIAverage.DSLU_App_Price = dslu_app_price;
   C1_DSLURSIAverage.DSLU_Signal_Period = dslu_signal_period;
   C1_DSLURSIAverage.DSLU_Zones = dslu_zones;
   C1_DSLURSIAverage.Tolerance = dslu_rsi_lookback_tolerance;
   C1_DSLURSIAverage.Is_Exit_Indicator = dslu_rsi_is_exit;
   
   C2_TrendFlexX2.Pair = pair;
   C2_TrendFlexX2.Timeframe = timeframe;
   C2_TrendFlexX2.Fast_Period = trendflex_fast_period;
   C2_TrendFlexX2.Slow_Period = trendflex_slow_period;
   C2_TrendFlexX2.Method = trendflex_method;
   C2_TrendFlexX2.Tolerance = trendflex_lookback_tolerance;
   C2_TrendFlexX2.Is_Exit_Indicator = trendflex_is_exit;
...
int CDualIndicatorTest::OnInitEvent(void){
   ...
   C1_DSLURSIAverage.Initialize();
   C2_TrendFlexX2.Initialize();
...
TRADING_TERMS CDualIndicatorTest::CheckConfirmationIncidcators(void){
   ...
   TRADING_TERMS c1_signal = C1_DSLURSIAverage.CheckSignal(1);
   TRADING_TERMS c2_signal = C2_TrendFlexX2.CheckSignal(1);
...
TRADING_TERMS CDualIndicatorTest::CheckExit(void){
   ...
   if (C1_DSLURSIAverage.Is_Exit_Indicator && !C2_TrendFlexX2.Is_Exit_Indicator){
      return C1_DSLURSIAverage.CheckSignal(1);
   }
   else if (C2_TrendFlexX2.Is_Exit_Indicator && !C1_DSLURSIAverage.Is_Exit_Indicator){
      return C2_TrendFlexX2.CheckSignal(1);
   }
   else {
      TRADING_TERMS c1_signal = C1_DSLURSIAverage.CheckSignal(1);
      TRADING_TERMS c2_signal = C2_TrendFlexX2.CheckSignal(1);
      
      if ((c1_signal == BUY_SIGNAL || c1_signal == BULLISH) && (c2_signal == BUY_SIGNAL || c2_signal == BULLISH)) return BULLISH;
      else if ((c1_signal == SELL_SIGNAL || c1_signal == BEARISH) && (c2_signal == SELL_SIGNAL || c2_signal == BEARISH)) return BEARISH;
   }
   return NO_SIGNAL;
}
//+------------------------------------------------------------------+
