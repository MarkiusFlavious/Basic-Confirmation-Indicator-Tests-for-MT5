...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Squeeze_Handle = iCustom(Pair,Timeframe,"Squeeze.ex5",Squeeze_Period,Dev_Multiplier,Atr_Multiplier);
...
//| - Squeeze value (buffer = 0) flips between 3 states:             |
//|   - Bullish = 1, bearish = -1, no signal/exit = 0                |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_value[];
   CopyBuffer(Squeeze_Handle,0,1,2,line_value);
