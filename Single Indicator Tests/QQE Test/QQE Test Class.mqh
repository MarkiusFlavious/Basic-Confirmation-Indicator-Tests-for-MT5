...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   QQE_RSI_Smoothing_Factor = qqe_rsi_smoothing_factor;
   QQE_WP_Fast = qqe_wp_fast;
   QQE_WP_Slow = qqe_wp_slow;
   QQE_Applied_Price = qqe_applied_price;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   QQE_Handle = iCustom(Pair,Timeframe,"QQE.ex5",QQE_RSI_Period,QQE_RSI_Smoothing_Factor,QQE_WP_Fast,QQE_WP_Slow,QQE_Applied_Price);
...
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   ...
   CopyBuffer(QQE_Handle,0,1,2,fast_line);
   CopyBuffer(QQE_Handle,1,1,2,slow_line);
