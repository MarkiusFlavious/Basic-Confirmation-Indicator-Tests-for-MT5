...
int OnInit()
  {
   ...
   ASARHandle = iCustom(_Symbol,Timeframe,"ASAR.ex5",InpPriceMode,InpAppliedPrice,InpStart,InpStep,InpMax,InpFilter,InpMinChange);
...
enTradeSignal CheckForSignal()
  {
   // ASAR UP Buffer is 0 and Down buffer is 1. Simple empty value switching.
   double SARUpValues[];
   CopyBuffer(ASARHandle,0,1,2,SARUpValues);
   ArrayReverse(SARUpValues);
   
   double SARDownValues[];
   CopyBuffer(ASARHandle,1,1,2,SARDownValues);
   ArrayReverse(SARDownValues);
   
   if ((SARUpValues[0] != EMPTY_VALUE) && (SARDownValues[0] == EMPTY_VALUE))
   {
    return buy_signal;
   }
   if ((SARDownValues[0] != EMPTY_VALUE) && (SARUpValues[0] == EMPTY_VALUE))
   {
    return sell_signal;
   }
