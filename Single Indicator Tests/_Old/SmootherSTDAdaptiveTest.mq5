...
int OnInit()
  {
   ...
   SmootherAdpativeSTDHandle = iCustom(_Symbol,Timeframe,"smoother_std_adaptive_4.ex5",SmtPeriod,inpASTDPrice,AdaptivePeriod,DeviationType,ColorSteps,ColorFrom,ColorTo);
...
enTradeSignal CheckForSignal()
  {
   // What's interesting here is that you can filter out certain colour values.
   // The colour values returned will be a number from 0 to the step value.
   // Eg. On default 50 steps. 24 to 0 is more orange and 25 to 50 is more green.
   // For now we'll just use the switch as a signal.
   // Colour Buffer at 1.
   
   double ColourValues[];
   CopyBuffer(SmootherAdpativeSTDHandle,1,1,2,ColourValues);
   ArrayReverse(ColourValues);
   
   if ( (ColourValues[0] > (ColorSteps / 2)) && (ColourValues[1] <= (ColorSteps / 2)) )
   {
    return buy_signal;
   }
   if ( (ColourValues[0] < (ColorSteps / 2)) && (ColourValues[1] >= (ColorSteps / 2)) )
