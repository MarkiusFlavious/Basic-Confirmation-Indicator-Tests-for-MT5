...
input group "PTL 2 Inputs"
input int inpPTLFastLength = 3; // Fast length
input int inpPTLSlowLength = 7; // Slow length
...
int OnInit()
  {
   ...
   PTLHandle = iCustom(_Symbol,Timeframe,"Downloads\\PTL (2).ex5",inpPTLFastLength,inpPTLSlowLength);
...
enTradeSignal CheckForSignal()
  {
   // PTL bar colour buffer is at 4. 0 is blue, 1 is red and -1 is not coloured.
   double BarColourValues[];
   CopyBuffer(PTLHandle,4,1,2,BarColourValues);
   ArrayReverse(BarColourValues);
   
   if ( (BarColourValues[0] == 0) && ( (BarColourValues[1] == 1) || (BarColourValues[1] == -1) ) )
   {
    return buy_signal;
   }
   if ( (BarColourValues[0] == 1) && ( (BarColourValues[1] == 0) || (BarColourValues[1] == -1) ) )
   {
    return sell_signal;
   }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   if (inTrade == true)
   {
    double BarColourValue[];
    CopyBuffer(PTLHandle,4,1,1,BarColourValue);
   
    if (PositionSelectByTicket(posTicket))
    {
     
     if (BarColourValue[0] == -1)
     {
