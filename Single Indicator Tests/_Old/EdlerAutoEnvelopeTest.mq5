...
input enPrices inpAppliedPrice = pr_close; // Applied Price
input double inpDeviationsFactor = 2.7; // Deviation factor
input int inpDeviationsPeriod = 100; // Deviation Period
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   ElderAutoHandle = iCustom(_Symbol,Timeframe,"elder_auto_envelope_1_2.ex5",inpSlowEmaPeriod,inpFastEmaPeriod,inpAppliedPrice,inpDeviationsFactor,inpDeviationsPeriod);
...
enTradeSignal CheckForSignal()
  {
   ...
   CopyBuffer(ElderAutoHandle,4,1,2,FastEMAValues);
   ArrayReverse(FastEMAValues);
   
   double SlowEMAValues[];
   CopyBuffer(ElderAutoHandle,2,1,2,SlowEMAValues);
   ArrayReverse(SlowEMAValues);
   
   if ((FastEMAValues[0] > SlowEMAValues[0]) && (FastEMAValues[1] < SlowEMAValues[1]))
   {
    return buy_signal;
   }
   if ((FastEMAValues[0] < SlowEMAValues[0]) && (FastEMAValues[1] > SlowEMAValues[1]))
