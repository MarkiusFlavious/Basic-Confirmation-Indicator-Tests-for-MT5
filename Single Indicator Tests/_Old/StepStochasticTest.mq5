...
input group "Step Stochastic Inputs"
input ENUM_APPLIED_PRICE inpStepStochPrice = PRICE_MEDIAN; // Price Calculation
input int inpStepStochAtrPeriod = 10; // ATR Period
input double inpStepStochKSlow = 1.0; // K slow 
input double inpStepStochKFast = 1.0; // K fast
input int inpStepStochWindow = 256; // ATR minimum maximum window size
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   StepStochHandle = iCustom(_Symbol,Timeframe,"step_stochastic.ex5",inpStepStochPrice,inpStepStochAtrPeriod,inpStepStochKSlow,inpStepStochKFast,inpStepStochWindow);
...
enTradeSignal CheckForSignal()
  {
   double StepStochFastValues[];
   CopyBuffer(StepStochHandle,2,1,2,StepStochFastValues);
   ArrayReverse(StepStochFastValues);
   
   double StepStochSlowValues[];
   CopyBuffer(StepStochHandle,3,1,2,StepStochSlowValues);
   ArrayReverse(StepStochSlowValues);
   
   if ((StepStochFastValues[0] > StepStochSlowValues[0]) && (StepStochFastValues[1] < StepStochSlowValues[1]))
   {
    return buy_signal;
   }
   if ((StepStochFastValues[0] < StepStochSlowValues[0]) && (StepStochFastValues[1] > StepStochSlowValues[1]))
