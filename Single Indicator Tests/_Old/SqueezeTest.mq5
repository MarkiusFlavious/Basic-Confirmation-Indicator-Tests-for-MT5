...
input group "Squeeze Inputs"
input int inpSqPeriod = 20; // Period
...
int OnInit()
  {
   ...
   SqueezeHandle = iCustom(_Symbol,Timeframe,"Squeeze.ex5",inpSqPeriod,inpDevMultiplier,inpAtrMultiplier);
...
enTradeSignal CheckForSignal()
  {
   // 1 is bullish, -1 is bearish and 0 is ranging or exit.
   double SqueezeValues[];
   CopyBuffer(SqueezeHandle,0,1,2,SqueezeValues);
   ArrayReverse(SqueezeValues);
   
   if ((SqueezeValues[0] == 1) && ((SqueezeValues[1] == -1) || (SqueezeValues[1] == 0)))
   {
    return buy_signal;
   }
   if ((SqueezeValues[0] == -1) && ((SqueezeValues[1] == 1) || (SqueezeValues[1] == 0)))
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   if (inTrade == true)
   {
    ...
    {
     double SqueezeValue[];
     CopyBuffer(SqueezeHandle,0,1,1,SqueezeValue);
