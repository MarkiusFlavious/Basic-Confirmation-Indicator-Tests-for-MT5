...
int OnInit()
  {
   ...
   DSLMACDHandle = iCustom(_Symbol,Timeframe,"Dsl MACD.ex5",inpFastEma,inpSlowEma,inpSignalPeriod,inpPrice,inpZones);
...
enTradeSignal CheckForSignal()
  {
   ...
   CopyBuffer(DSLMACDHandle,5,1,2,ColourLineValues);
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   if (inTrade == true)
   {
    if (PositionSelectByTicket(posTicket))
    {
     ...
     CopyBuffer(DSLMACDHandle,5,1,1,ColourLineValue);
