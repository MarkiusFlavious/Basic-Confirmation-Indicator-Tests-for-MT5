...
int OnInit()
  {
   ...
   DSSBressertHandle = iCustom(_Symbol,Timeframe,"dssbressert.ex5",EMA_period,Sto_period,Shift);
...
enTradeSignal CheckForSignal()
  {
   // Ignore Over-sold/Over-bought and just take lines crossing as a signal.
   // Buffer 0 is the faster signal line. Indi turns blue when it's above the slower line.
   // Indi turns pink when the fast line is below the slow line.
   double FastLineValues[];
   CopyBuffer(DSSBressertHandle,0,1,2,FastLineValues);
   ArrayReverse(FastLineValues);
   
   double SlowLineValues[];
   CopyBuffer(DSSBressertHandle,1,1,2,SlowLineValues);
