...
enum enStratType
  {
   two_line, // 2 Line Cross
   slow_zero, // Slow Line Zero Cross
   fast_zero // Fast Line Zero Cross
  };
...
int OnInit()
  {
   ...
   TrendFlexHandle = iCustom(_Symbol,Timeframe,"TrendFlex x 2.ex5",inpFastPeriod,inpSlowPeriod);
...
//+------------------------------------------------------------------+
enTradeSignal CheckForSignal(enStratType StrategyType)
  {
   // Fast/Blue Line Buffer 0. Slow/Red Line Buffer 1
   double FastLineValues[];
   CopyBuffer(TrendFlexHandle,0,1,2,FastLineValues);
   ArrayReverse(FastLineValues);
   
   double SlowLineValues[];
   CopyBuffer(TrendFlexHandle,1,1,2,SlowLineValues);
   ArrayReverse(SlowLineValues);
   
   // As 2 Line Cross
   if (StrategyType == two_line)
   {
    if ((FastLineValues[0] > SlowLineValues[0]) && (FastLineValues[1] < SlowLineValues[1]))
    {
     return buy_signal;
    }
    if ((FastLineValues[0] < SlowLineValues[0]) && (FastLineValues[1] > SlowLineValues[1]))
   ...
   // As Slow Line Zero Cros
   if (StrategyType == slow_zero)
   {
    if ((SlowLineValues[0] > 0) && (SlowLineValues[1] < 0))
    {
     return buy_signal;
    }
    if ((SlowLineValues[0] < 0) && (SlowLineValues[1] > 0))
   ...
   // As Fast Line Zero Cross
   if (StrategyType == fast_zero)
   {
    if ((FastLineValues[0] > 0) && (FastLineValues[1] < 0))
    {
     return buy_signal;
    }
    if ((FastLineValues[0] < 0) && (FastLineValues[1] > 0))
