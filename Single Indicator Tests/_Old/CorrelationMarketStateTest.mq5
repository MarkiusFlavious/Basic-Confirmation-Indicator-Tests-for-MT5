...
input group "Correlation Market State Input"
input int  inpPeriod = 35; // Period
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
int ATRChannelHandle;
int CorrelationMarketStateHandle;
//+------------------------------------------------------------------+
//| Global Variables                                                 |
//+------------------------------------------------------------------+
int barTotal;
...
int OnInit()
  {
   ...
   CorrelationMarketStateHandle = iCustom(_Symbol,Timeframe,"Correlation market state.ex5",inpPeriod);
...
enTradeSignal CheckForSignal()
  {
   // Buffer 2. 1 is buy, -1 is sell, 0 is exit or do nothing.
   double CorrMarketStateSignalValues[];
   CopyBuffer(CorrelationMarketStateHandle,2,1,2,CorrMarketStateSignalValues);
   ArrayReverse(CorrMarketStateSignalValues);
   
   if ((CorrMarketStateSignalValues[0] == 1) && ((CorrMarketStateSignalValues[1] == -1) || (CorrMarketStateSignalValues[1] == 0)))
   {
    return buy_signal;
   }
   if ((CorrMarketStateSignalValues[0] == -1) && ((CorrMarketStateSignalValues[1] == 1) || (CorrMarketStateSignalValues[1] == 0)))
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   if (inTrade == true)
   {
    if (PositionSelectByTicket(posTicket))
    {
     ...
     double CorrMarketStateSignal[];
     CopyBuffer(CorrelationMarketStateHandle,2,1,1,CorrMarketStateSignal);
     
     if (CorrMarketStateSignal[0] == 0)
     {
