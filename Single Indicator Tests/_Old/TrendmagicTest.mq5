...
input group "Trendmagic Inputs"
input int inpTrendMagicCCIPeriod = 50; // CCI Period
input int inpTrendMagicATRPeriod = 5; // ATR Period
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
int ATRChannelHandle;
int TrendMagicHandle;
//+------------------------------------------------------------------+
//| Global Variables                                                 |
//+------------------------------------------------------------------+
int barTotal;
...
int OnInit()
  {
   ...
   TrendMagicHandle = iCustom(_Symbol,Timeframe,"Downloads\\trendmagic.ex5",inpTrendMagicCCIPeriod,inpTrendMagicATRPeriod);
...
enTradeSignal CheckForSignal()
  {
   // Colour at buffer pos 1. 1 is bearish, 0 is bullish.
   double TMColourValues[];
   CopyBuffer(TrendMagicHandle,1,1,2,TMColourValues);
   ArrayReverse(TMColourValues);
   
   if ((TMColourValues[0] == 0) && (TMColourValues[1] == 1))
   {
    return buy_signal;
   }
   if ((TMColourValues[0] == 1) && (TMColourValues[1] == 0))
   {
    return sell_signal;
   }
