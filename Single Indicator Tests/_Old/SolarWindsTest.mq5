...
input group "Solar Winds Inputs"
input ENUM_ON_OFF       NRP = ENUM_ON;  // Non-Repainting (NPR) ON/OFF
...
int OnInit()
  {
   ...
   SolarWindsHandle = iCustom(_Symbol,Timeframe,"Solar Winds.ex5",NRP,UseEvents,period,smooth,DoAlert,alertMail,lngLineClr,shrtLineClr,lngHistClor,shrtHistClor,Uniquifier);
...
enTradeSignal CheckForSignal()
  {
   // Colour change again. Buffer 1. 0 is green and 1 is red.
   double ColourLineValues[];
   CopyBuffer(SolarWindsHandle,1,1,2,ColourLineValues);
   ArrayReverse(ColourLineValues);
   
   if ((ColourLineValues[0] == 0) && (ColourLineValues[1] == 1))
   {
    return buy_signal;
   }
   if ((ColourLineValues[0] == 1) && (ColourLineValues[1] == 0))
   {
    return sell_signal;
   }
