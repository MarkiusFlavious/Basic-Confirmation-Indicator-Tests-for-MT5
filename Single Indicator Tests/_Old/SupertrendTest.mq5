...
input int    inpSuperPeriod = 10; // Supertrend Period
input double inpSuperMultiplier = 3; // Supertrend Multiplier
input bool   inpSuperShowFilling = false; // Supertrend Show Filling
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   SuperHandle = iCustom(_Symbol,Timeframe,"supertrend.ex5",inpSuperPeriod,inpSuperMultiplier,inpSuperShowFilling);
...
enTradeSignal CheckForSignal()
  {
   double SuperColour[]; // 0 is green, 1 is red
   CopyBuffer(SuperHandle,3,1,2,SuperColour);
   ArrayReverse(SuperColour);
   
   if ((SuperColour[0] == 0) && (SuperColour[1] == 1))
   {
    return buy_signal;
   }
   if ((SuperColour[0] == 1) && (SuperColour[1] == 0))
   {
    return sell_signal;
   }
