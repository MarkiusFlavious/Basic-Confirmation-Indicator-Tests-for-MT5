...
//+-----------------------Half Trend Inputs--------------------------+
input group "Half Trend Inputs"
input int inpHTAmplitude = 5; // Half Trend Amplitude
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   // The indicator may have 11 inputs but only one matters. Alerts are also set to off. 
   // Interesting Discovery: You need to pass values for every input group as they appear in the indicator. It does seem to not matter what values 
   //                        get passed into those fields. Just as long as they're there.
   HalfTrendHandle = iCustom(_Symbol,Timeframe,"Half Trend New Alert.ex5",inpHTAmplitude,"Arrow",233,234,10,"Alerts","alert.wav",3,3,false,false,false,false);
...
enTradeSignal CheckForSignal()
  {
   // For half trend 1 is blue(bullish) and 0 is red(bearish.) The buffer position for colour is 1.
   double HalfTrendColourValues[];
   CopyBuffer(HalfTrendHandle,1,1,2,HalfTrendColourValues);
   ArrayReverse(HalfTrendColourValues);
   
   if ((HalfTrendColourValues[0] == 1) && (HalfTrendColourValues[1] == 0))
   {
    return buy_signal;
   }
   if ((HalfTrendColourValues[0] == 0) && (HalfTrendColourValues[1] == 1))
   {
    return sell_signal;
   }
