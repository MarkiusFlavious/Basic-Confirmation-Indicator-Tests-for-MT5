...
enum SASTD_PRICES {
   ...
   PR_MEDIAN_B, // Average median body (open+close)/2
   PR_TREND_BIASED, // Trend biased price
   PR_TREND_BIASED_EXTREME, // Trend biased (extreme) price
   ...
   PR_HA_MEDIAN_B, // Heiken ashi median body
   PR_HAT_BIASED, // Heiken ashi trend biased price
   PR_HAT_BIASED_EXTREME // Heiken ashi trend biased (extreme) price
};

enum SASTD_METHODS
{
   STD_CUST_SAM, // Custom - with sample correction
   STD_CUST_NOSAM // Custom - without sample correction
};
//+------------------------------------------------------------------+
...
// Input Parameters:
   ...
   //STD Adaptive Inputs:
   double SMT_Period;
   SASTD_PRICES STD_Price;
   int Adaptive_Period;
   SASTD_METHODS Deviation_Type;
   int Start_Range;
   int End_Range;

// Indicator Handles:
   int ATR_Channel_Handle;
   int Sm_STD_Adaptive_Handle;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Sm_STD_Adaptive_Handle = iCustom(Pair,Timeframe,"smoother_std_adaptive_4.ex5",SMT_Period,STD_Price,Adaptive_Period,Deviation_Type,50,clrOrange,clrLime);
...
//| - Colour change but on a gradient from 0 to 50                   |
//|   - We can filter out the middle values                          |
//|   - Colour buffer = 1                                            | 
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double colour_value[];
   CopyBuffer(Sm_STD_Adaptive_Handle,1,1,2,colour_value);
   ArrayReverse(colour_value);
   
   if (colour_value[0] >= End_Range){
      if (colour_value[1] < End_Range) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (colour_value[0] <= Start_Range){
      if (colour_value[1] > Start_Range) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
