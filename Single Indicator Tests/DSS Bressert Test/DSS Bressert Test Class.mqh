...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   DSS_Bressert_Handle = iCustom(Pair,Timeframe,"dssbressert.ex5",DSS_EMA_Period,Stoch_Period,Shift);
...
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   ...
   CopyBuffer(DSS_Bressert_Handle,0,1,2,fast_line);
   CopyBuffer(DSS_Bressert_Handle,1,1,2,slow_line);
