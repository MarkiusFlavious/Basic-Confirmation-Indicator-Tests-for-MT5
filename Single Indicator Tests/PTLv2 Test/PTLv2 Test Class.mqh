...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   PTL_Fast_Length = ptl_fast_length;
   PTL_Slow_Length = ptl_slow_length;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   PTL2_Handle = iCustom(Pair,Timeframe,"Downloads\\PTL (2).ex5",PTL_Fast_Length,PTL_Slow_Length);
...
//| - Candle colour change buffer = 4:                               |
//|   - Blue = 0, red = 1, no colour = -1                            |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double bar_colour[];
   CopyBuffer(PTL2_Handle,4,1,2,bar_colour);
   ArrayReverse(bar_colour);
   
   if (bar_colour[0] == 0){
      if (bar_colour[1] == -1 || bar_colour[1] == 1) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (bar_colour[0] == 1){
      if (bar_colour[1] == -1 || bar_colour[1] == 0) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
