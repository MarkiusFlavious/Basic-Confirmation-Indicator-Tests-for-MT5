...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   TTF_T3_Period = ttf_t3_period;
   TTF_T3_Hot = ttf_t3_hot;
   TTF_T3_Type = ttf_t3_type;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   TTF_Handle = iCustom(Pair,Timeframe,"Downloads\\Trend trigger factor.ex5",TTF_Period,TTF_T3_Period,TTF_T3_Hot,TTF_T3_Type);
...
//| - Ignore everything else and just use the slower line as a       |
//|   zero cross:                                                    |
//:   - Levels line buffer = 2                                       |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_value[];
   CopyBuffer(TTF_Handle,2,1,2,line_value);
   ArrayReverse(line_value);
   
   if (line_value[0] > 0){
      if (line_value[1] < 0) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (line_value[0] < 0){
      if (line_value[1] > 0) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
