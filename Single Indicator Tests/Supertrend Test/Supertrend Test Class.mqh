...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Supertrend_Handle = iCustom(Pair,Timeframe,"supertrend.ex5",Super_Period,Super_Multiplier,false);
...
//| - Supertrend colour buffer = 3, value buffer = 2                 |
//|   - Green = 0, red = 1                                           |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_colour[];
   CopyBuffer(Supertrend_Handle,3,1,2,line_colour);
...
void CSingleIndicatorTester::TrailStop(void){
   if (TradePosition.Trail_Stop && TradePosition.Modified){
      ...
      CopyBuffer(Supertrend_Handle,2,1,1,supertrend_value);
      int digits = (int)SymbolInfoInteger(Pair,SYMBOL_DIGITS);
      double new_stop;
