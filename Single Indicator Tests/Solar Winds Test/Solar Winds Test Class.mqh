...
// Input Parameters:
   ...
   // Solar Winds NRP Inputs:
   int SW_Period;
   int SW_Smooth;
   
// Indicator Handles:
   int ATR_Channel_Handle;
   int Solar_Winds_Handle;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Solar_Winds_Handle = iCustom(Pair,Timeframe,"Solar Winds.ex5",ENUM_ON,ENUM_OFF,SW_Period,SW_Smooth,false);
...
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   ...
   double bar_colour[];
   CopyBuffer(Solar_Winds_Handle,1,1,2,bar_colour);
