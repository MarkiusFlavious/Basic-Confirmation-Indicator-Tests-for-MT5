...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
                                               ...
                                               ENUM_APPLIED_PRICE ma_applied_price,
                                               double signal_period,
                                               DSL_DISPLAY zones){ // Add Indicator Inputs
   // Initialize Inputs
   Pair = pair;
   Timeframe = timeframe;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   DSL_CCI_Handle = iCustom(Pair,Timeframe,"Dsl CCI of average.ex5",CCI_Period,MA_Period,MA_Method,MA_Applied_Price,Signal_Period,Zones);
...
//| - Line Colour Change, buffer value is 5:                         |
//|   - Gray = 0, Green = 1, Orange = 2                              |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_colour[];
   CopyBuffer(DSL_CCI_Handle,5,1,2,line_colour);
