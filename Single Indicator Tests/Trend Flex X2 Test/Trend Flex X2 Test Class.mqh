...
enum TREND_FLEX_MODES {
   TWO_LINES_CROSS, // Two Line Cross
   ZERO_CROSS, // Slow Line Zero Cross
};
//+------------------------------------------------------------------+
...
// Input Parameters:
   ...
   // Trend Flex X2 Inputs:
   int Fast_Period;
   int Slow_Period;
   TREND_FLEX_MODES TF_Mode;

// Indicator Handles:
   int ATR_Channel_Handle;
   int Trend_Flex_Handle;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Trend_Flex_Handle = iCustom(Pair,Timeframe,"TrendFlex x 2.ex5",Fast_Period,Slow_Period);
...
//| - Two lines cross:                                               |
//|   - Fast line buffer = 0, slow line buffer = 1                   |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double fast_line[],slow_line[];
   CopyBuffer(Trend_Flex_Handle,0,1,2,fast_line);
   CopyBuffer(Trend_Flex_Handle,1,1,2,slow_line);
   ArrayReverse(fast_line);
   ArrayReverse(slow_line);
   
   if (TF_Mode == TWO_LINES_CROSS){
   ...
   else if (TF_Mode == ZERO_CROSS){
      if (slow_line[0] > 0){
         if (slow_line[1] < 0) return BUY_SIGNAL;
         else return BULLISH;
      }
      else if (slow_line[0] < 0){
         if (slow_line[1] > 0) return SELL_SIGNAL;
         else return BEARISH;
      }
   }
   return NO_SIGNAL;
