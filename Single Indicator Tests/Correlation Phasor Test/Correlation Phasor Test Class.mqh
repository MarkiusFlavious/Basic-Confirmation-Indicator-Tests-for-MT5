...
// Input Parameters:
   ...
   int  Cor_Phasor_Period;

// Indicator Handles:
   int ATR_Channel_Handle;
   int Cor_Phasor_Handle;
// Other Declarations:
   int Bar_Total;
   ulong Ticket_Number;
   bool In_Trade;
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   Cor_Phasor_Period = cor_phasor_handle;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Cor_Phasor_Handle = iCustom(Pair,Timeframe,"Correlation phasor.ex5",Cor_Phasor_Period);
...
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   ...
   double line_colour_value[];
   CopyBuffer(Cor_Phasor_Handle,3,1,2,line_colour_value);
   ArrayReverse(line_colour_value);
   
   if (line_colour_value[0] == 0){
      if (line_colour_value[1] == 1) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (line_colour_value[0] == 1){
      if (line_colour_value[1] == 0) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
