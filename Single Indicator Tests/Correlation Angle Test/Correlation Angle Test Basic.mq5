...
input group "Correlation Angle Input"
input int  input_cor_angle_period = 35; // Period
//+------------------------------------------------------------------+
//| Handles:                                                         |
//+------------------------------------------------------------------+
int ATR_Channel_Handle{};
int Cor_Angle_Handle{};
//+------------------------------------------------------------------+
//| Other Globals:                                                   |
//+------------------------------------------------------------------+
int Bar_Total{};
...
int OnInit(){
   ...
   Cor_Angle_Handle = iCustom(_Symbol,input_timeframe,"Correlation Angle.ex5",input_cor_angle_period);
...
//| - 2 line cross:                                                  |
//|   - Green line value buffer = 2, red line value buffer = 3       |
//+------------------------------------------------------------------+
TRADING_TERMS LookForSignal(){
   
   double green_line_value[],red_line_value[];
   CopyBuffer(Cor_Angle_Handle,2,1,2,green_line_value);
   CopyBuffer(Cor_Angle_Handle,3,1,2,red_line_value);
   ArrayReverse(green_line_value);
   ArrayReverse(red_line_value);
   
   if (green_line_value[0] > red_line_value[0]){
      if (green_line_value[1] < red_line_value[1]) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (red_line_value[0] > green_line_value[0]){
      if (red_line_value[1] < green_line_value[1]) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
