...
int OnInit(){
   ...
   DiNapoli_Stoch_Handle = iCustom(_Symbol,input_timeframe,"dinapolistochastic.ex5",input_fastK_period,input_slowK_period,input_slowD_period,0);
...
//| - There are other ways to use this indicator:                    |
//|   - I'm using the slower signal line as a "0 cross" indicator    |
//|   - In this case it's actually a 50 cross                        |
//|   - The buffer value = 1                                         |
//+------------------------------------------------------------------+
TRADING_TERMS LookForSignal(){
   
   double signal_line[];
   CopyBuffer(DiNapoli_Stoch_Handle,1,1,2,signal_line);
   ArrayReverse(signal_line);
   
   if (signal_line[0] > 50){
      if (signal_line[1] < 50) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (signal_line[0] < 50){
      if (signal_line[1] > 50) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
