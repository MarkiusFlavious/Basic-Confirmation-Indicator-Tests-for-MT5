...
// Input Parameters:
   ...
   // Gann High Low Activator Input:
   int Activator_Period;

// Indicator Handles:
   int ATR_Channel_Handle;
   int Gann_HL_Activator_Handle;
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   Activator_Period = activator_period;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Gann_HL_Activator_Handle = iCustom(Pair,Timeframe,"Gann high low activator (oscillator).ex5",Activator_Period);
...
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   ...
   double line_colour[];
   CopyBuffer(Gann_HL_Activator_Handle,1,1,2,line_colour);
