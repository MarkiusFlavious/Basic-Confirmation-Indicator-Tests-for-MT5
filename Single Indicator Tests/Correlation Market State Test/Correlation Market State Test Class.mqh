...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
                                               ...
                                               bool move_stop,
                                               bool trail_stop,
                                               int cor_state_period){ // Add Indicator Inputs
   // Initialize Inputs
   Pair = pair;
   Timeframe = timeframe;
   ...
   Cor_State_Period = cor_state_period;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Cor_State_Handle = iCustom(Pair,Timeframe,"Correlation market state.ex5",Cor_State_Period);
...
//| - Line value at buffer 2 flips between 3 states:                 |
//|   - 1 is bullish, -1 is bearish, 0 is exit/no signal             |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   double line_value[];
   CopyBuffer(Cor_State_Handle,2,1,2,line_value);
   ArrayReverse(line_value);
   
   if (line_value[0] == 1){
      if (line_value[1] == 0 || line_value[1] == -1) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (line_value[0] == -1){
      if (line_value[1] == 0 || line_value[1] == 1) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
