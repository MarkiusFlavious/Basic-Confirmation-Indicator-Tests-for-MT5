...
// Input Parameters:
   ...
   // KRI Inputs:
   int KRI_Period;
   ENUM_MA_METHOD KRI_Method;
   double KRI_Ratio;
   KRI_APPLIED_PRICE KRI_App_Price;
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   KRI_Period = kri_period;
   KRI_Method = kri_method;
   KRI_Ratio = kri_ratio;
   KRI_App_Price = kri_app_price;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   KRI_Handle = iCustom(Pair,Timeframe,"kri.ex5",KRI_Period,KRI_Method,KRI_Ratio,KRI_App_Price);
...
//| - 0 cross, kri line buffer = 0                                   |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_value[];
   CopyBuffer(KRI_Handle,0,1,2,line_value);
