...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   KF_Timeframe = kf_timeframe;
   KF_Period = kf_period;
   KF_App_Price = kf_app_price;
   KF_Interpolate= kf_interpolate;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Kalman_Filter_Handle = iCustom(Pair,Timeframe,"Kalman filter (mtf).ex5",KF_Timeframe,KF_Period,KF_App_Price,KF_Interpolate);
...
//| - Line colour change buffer = 1 :                                |
//|   - Green = 1, Red = 2                                           |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_colour[];
   CopyBuffer(Kalman_Filter_Handle,1,1,2,line_colour);
   ArrayReverse(line_colour);
   
   if (line_colour[0] == 1){
      if (line_colour[1] == 2) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (line_colour[0] == 2){
      if (line_colour[1] == 1) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
