...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
                                               ...
                                               Smooth_Method smoothing_method,
                                               int smoothing_length,                   
                                               int smoothing_phase){ // Add Indicator Inputs
   // Initialize Inputs
   Pair = pair;
   Timeframe = timeframe;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   HA_Smoothed_Handle = iCustom(Pair,Timeframe,"heiken_ashi_smoothed.ex5",Smoothing_Method,Smoothing_Length,Smoothing_Phase);
...
//| - Colour change buffer = 4 :                                     |
//|   - Blue = 0, Red = 1                                            |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double bar_colour[];
   CopyBuffer(HA_Smoothed_Handle,4,1,2,bar_colour);
   ArrayReverse(bar_colour);
   
   if (bar_colour[0] == 0){
      if (bar_colour[1] == 1) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (bar_colour[0] == 1){
      if (bar_colour[1] == 0) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
