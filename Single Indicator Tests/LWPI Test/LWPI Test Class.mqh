...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   LWPI_Handle = iCustom(Pair,Timeframe,"LWPI.ex5",LWPI_Period);
...
//| - Used as an upside down 50 cross:                               |
//|   - Line value buffer = 0                                        |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double line_value[];
   CopyBuffer(LWPI_Handle,0,1,2,line_value);
   ArrayReverse(line_value);
   
   if (line_value[0] < 50){
      if (line_value[1] > 50) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (line_value[0] > 50){
      if (line_value[1] < 50) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
