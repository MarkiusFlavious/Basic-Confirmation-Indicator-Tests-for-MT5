...
enum TREND_REFLEX_METHOD {
   TWO_LINES_CROSS, // 2 Line Cross
...
// Input Parameters:
   ...
   // Ehlers Trend Reflex Inputs:
   int TReflex_Length;
   TREND_REFLEX_METHOD Method;

// Indicator Handles:
   int ATR_Channel_Handle;
   int Trend_Reflex_Handle;
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
                                               ...
                                               bool trail_stop,
                                               int treflex_length,
                                               TREND_REFLEX_METHOD method){ // Add Indicator Inputs
   // Initialize Inputs
   Pair = pair;
   Timeframe = timeframe;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Trend_Reflex_Handle = iCustom(Pair,Timeframe,"EhlersTrendReflex.ex5",TReflex_Length);
...
//| - Two line cross or 0 cross:                                     |
//|   - Red line buffer = 0, blue line buffer = 1                    |
//|   - Blue line for 0 cross                                        |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double blue_line[], red_line[];
   CopyBuffer(Trend_Reflex_Handle,1,1,2,blue_line);
   CopyBuffer(Trend_Reflex_Handle,0,1,2,red_line);
   ArrayReverse(blue_line);
   ArrayReverse(red_line);
   
   if ( Method == ZERO_CROSS){
      if (blue_line[0] > 0){
         if (blue_line[1] < 0) return BUY_SIGNAL;
         else return BULLISH;
      }
      else if (blue_line[0] < 0){
         if (blue_line[1] > 0) return SELL_SIGNAL;
         else return BEARISH;
      }
   }
   else {
      if (blue_line[0] > red_line[0]){
         if (blue_line[1] < red_line[1]) return BUY_SIGNAL;
         else return BULLISH;
      }
      else if (blue_line[0] < red_line[0]){
         if (blue_line[1] > red_line[1]) return SELL_SIGNAL;
         else return BEARISH;
      }
   }
   return NO_SIGNAL;
