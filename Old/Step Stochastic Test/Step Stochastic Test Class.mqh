...
// Input Parameters:
   ...
   // Step Stochastic Inputs:
   ENUM_APPLIED_PRICE Step_Stoch_Price;
   int Step_Atr_Period;
   double K_Slow;
   double K_Fast;
...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   Step_Stoch_Price = step_stoch_price;
   Step_Atr_Period = step_atr_period;
   K_Slow = k_slow;
   K_Fast = k_fast;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   Step_Stoch_Handle = iCustom(Pair,Timeframe,"step_stochastic.ex5",Step_Stoch_Price,Step_Atr_Period,K_Slow,K_Fast);
...
//| - Two lines cross:                                               |
//|   - Fast line buffer = 2, slow line buffer = 3                   |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double fast_line[],slow_line[];
   CopyBuffer(Step_Stoch_Handle,2,1,2,fast_line);
   CopyBuffer(Step_Stoch_Handle,3,1,2,slow_line);
