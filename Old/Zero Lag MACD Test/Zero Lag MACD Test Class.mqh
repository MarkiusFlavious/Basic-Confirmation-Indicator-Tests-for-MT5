...
CSingleIndicatorTester::CSingleIndicatorTester(string pair,
   ...
   ZL_MACD_App_Price = zl_macd_app_price;
...
int CSingleIndicatorTester::OnInitEvent(void){
   ...
   ZL_MACD_Handle = iCustom(Pair,Timeframe,"zerolag_macd.ex5",Fast_Period,Slow_Period,Signal_Period,ZL_MACD_App_Price);
...
//| - We Simply use the red line as a zero cross:                    |
//|   - Buffer = 1                                                   |
//+------------------------------------------------------------------+
TRADING_TERMS CSingleIndicatorTester::LookForSignal(void){
   
   double signal_line[];
   CopyBuffer(ZL_MACD_Handle,1,1,2,signal_line);
   ArrayReverse(signal_line);
   
   if (signal_line[0] > 0){
      if (signal_line[1] < 0) return BUY_SIGNAL;
      else return BULLISH;
   }
   else if (signal_line[0] < 0){
      if (signal_line[1] > 0) return SELL_SIGNAL;
      else return BEARISH;
   }
   return NO_SIGNAL;
}
